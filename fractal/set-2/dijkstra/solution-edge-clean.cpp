...
void solve()
{
	...
	for(int i = 0; i < edges; i++)
	{
		...
		if(edge_map.count(mp(u,v)))
			edge_map[mp(u, v)] = min(edge_map[mp(u, v)], w);
		else
			edge_map[mp(u, v)] = w;
	}

	for(auto key_val : edge_map)
		graph.addEdge(key_val.first.first, key_val.first.second, key_val.second);
